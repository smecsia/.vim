...
function! s:factoryList(A,L,P)
  ...
  if rails#app().has('test')
    let found += rails#app().relglob("test/factories/","**/*",".rb")
  endif
  if rails#app().has('spec')
    let found += rails#app().relglob("spec/factories/","**/*",".rb")
  endif
  return s:autocamelize(found,a:A)
endfunction

function! s:riaappList(A,L,P)
  let found = []
  let found += rails#app().relglob("public/ria/src/app/","**/**","")
  return s:autocamelize(found,a:A)
endfunction
...
function! s:riaappEdit(cmd,...)
  return s:EditSimpleRb(a:cmd,"riaapp",a:0? a:1 : "application","public/ria/src/app/","",1)
endfunction
...
function! s:factoryEdit(cmd,...)
  ...
  let mapping = {'test': ['test/factories/','.rb'], 'spec': ['spec/factories/','.rb']}
  ...
  return s:EditSimpleRb(a:cmd,"factory",f.jump,tests[0][0],tests[0][1],1)
endfunction
